...
class TestAdderResource : public EDS DelegateResourceManager<int, int> {
    public:
        ...
        bool
        equals(const EDS Resource &other) const noexcept override {
                return false;
        }
        void
        operator()(int a, int b) noexcept override { }
        size_t
        get_subscriber_id() const noexcept override {
                return 0;
        }
        long
        get_use_count() const noexcept override {
                return 0;
        }
};
...
EDS_GTESTF(CallHandlerUGTest, UT02, CallHandlerTF, VoidNoparamsNoexcept) {
        ...
        EDS_INFO(TYPES) << "Call Back Function Type: " << typeid(IncrementTestWrapper_t::CallBack_t).name()
                        << EDS_EOL();
        const bool expected_CallBack_type_comparison_result = true;
        bool actual_CallBack_type_comparison_result =
...
EDS_GTESTF(CallHandlerUGTest, UT03, CallHandlerTF, MemberVoidParamsNoexcept) {
        ...
        EDS_INFO(TYPES) << "Call Back Function Type: " << typeid(AdderMemberTestWrapper_t::CallBack_t).name()
                        << EDS_EOL();
        const bool expected_CallBack_type_comparison_result = true;
        bool actual_CallBack_type_comparison_result =
