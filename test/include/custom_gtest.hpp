...
#endif
/// Informational message in a test goes to std::cerr because most test runners ignore std::cout unless the test fails and trace is on
#define EDS_INFO_OUT std::cerr
/// Error message in a test goes to std::cout because most test runners capture std::cout only when there is a failure.
...
/// A macro to define a probe in a test case, the probe is a message that is printed to the console and the test runner
/// 
/// The goal of the probe showing up during normal tests is to be able to diagnose the test case without having to run the test in debug mode, of in a debugger. 85% of cases can be diagnosed this way.
/// @param PASSERTION The embedded assertion executed by the probe. Can be any valid GTEST assertion.
...
#define EDS_PROBE(PASSERTION, ...)                                                                              \
        EDS_INFO(PROBE) << '(' << __LINE__ << ')' << #PASSERTION << ' ' __VA_ARGS__ << EDS_EOL();               \
        PASSERTION
/// An enhanced version of GTEST's TEST_F macro that allows to define a test case with a meaningful name
/// 
