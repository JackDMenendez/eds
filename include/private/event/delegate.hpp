...
template <class...PARMS> class Delegate<void(PARMS...) noexcept> : public DelegateResourceManager<PARMS...> {
        public:
                ...
                constexpr Delegate(void (*callback)(PARMS...) noexcept) noexcept
                      : m_managed_CallHandler(std::make_shared<CallHandler<FunctionPointer,PARMS...>>(callback,this)) {}
                template<class CALLBACK>
                constexpr Delegate(CALLBACK &&callback) noexcept 
                      : m_managed_CallHandler(
                            std::make_shared<CallHandler<FunctionPointer,PARMS...>>(std::forward<CALLBACK>(callback),this)) {}
                constexpr Delegate(const Delegate &delegate) noexcept : Delegate(delegate.m_managed_CallHandler) {}
                constexpr Delegate(Delegate &&delegate) noexcept : m_managed_CallHandler(std::move(delegate.m_managed_CallHandler)) {}
                bool equals(const Resource& other) const noexcept override { return get_subscriber_id() == other.get_subscriber_id();}
                constexpr Delegate &
