...
public:
  ...
  constexpr CallHandler(CALLBACK &call_back,
                        delegate_Resource_manager_t *d) noexcept
      : m_call_back(std::forward<CALLBACK>(call_back)),
        m_resource_id((size_t)&m_call_back), m_resource_manager(d) {}
  Resource &get_delegate() const noexcept override {
    return *m_resource_manager;
  }
  void set_delegate(Resource *delegate) noexcept override {
...
public:
  template <typename CALLBACK>
    requires regular_function<CALLBACK, PARAMS...>
  constexpr CallHandler(CALLBACK &&call_back,
                        delegate_Resource_manager_t *d) noexcept
...
template <class... PARAMS>
class CallHandler<MemberPointer, PARAMS...> : public Subscriber<PARAMS...> {
public:
  using CallBack_t = void(PARAMS...) noexcept;
  using Subscriber_t = Subscriber<PARAMS...>;
  using delegate_Resource_manager_t = DelegateResourceManager<PARAMS...>;
...
public:
  ...
  constexpr CallHandler(const CLASS *class_instance,
                        ...
                        delegate_Resource_manager_t *d) noexcept
      : m_call_back(std::make_shared<MemberCall<VoidReturnCode, Constant,
                                                NoException, CLASS, PARAMS...>>(
  ...
  constexpr CallHandler(CLASS *class_instance,
                        ...
                        delegate_Resource_manager_t *d) noexcept
      : m_call_back(std::make_shared<MemberCall<VoidReturnCode, NotConstant,
                                                NoException, CLASS, PARAMS...>>(
            class_instance, call_back)),
        m_resource_manager(d) {
    m_resource_id = m_call_back->hash();
...
template <> class CallHandler<MemberPointer> : public Subscriber<> {
public:
...
public:
  ...
  constexpr CallHandler(const CLASS *class_instance,
                        ...
                        delegate_Resource_manager_t *d) noexcept
      : m_call_back(
            new MemberCall<VoidReturnCode, Constant, NoException, CLASS>(
                class_instance, call_back)),
        m_resource_manager(d) {
    m_resource_id = m_call_back->hash();
  }
  template <typename CLASS>
    requires SomeClassType<CLASS>
  constexpr CallHandler(CLASS *class_instance,
                        void (CLASS::*call_back)() noexcept,
                        delegate_Resource_manager_t *d) noexcept
      : m_call_back(
            new MemberCall<VoidReturnCode, NotConstant, NoException, CLASS>(
                class_instance, call_back)),
        m_resource_manager(d) {
    m_resource_id = m_call_back->hash();
  }
...
class CallHandler<LambdaRef, LAMBDA> : public Subscriber<> {
public:
